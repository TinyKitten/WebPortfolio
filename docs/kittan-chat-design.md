...
| 3   | 絶対に暴言・公序良俗に反する発言をしない                            | fail-closed の多層ガードレール。全層を通過するまで一切返さない(§6)                          |
...
②で止まった場合はモデルを一切呼ばず、③〜⑤のどこで止まっても**同一の定型お断り文**を返します
...
interactions.create({
  ...
  generation_config: {
    ...
    max_output_tokens, // 既定640。暴走出力の上限
  },
...
Interactions API では**カスタム安全設定(safety_settings の閾値指定)がまだ使えない**ため、
モデル既定のフィルタに依存せず、自前の防御で「絶対に」を担保する必要があります。
想定する脅威は次のとおりです。
...
**JSONとして読めない・想定外の値・呼び出し例外のすべてをブロック扱い**にします。
...
| 安全設定         | 自前の多層ガードレール                   | SDKの `safety_settings`(型上は存在): Interactions APIで未サポートのため依存しない。既定フィルタは「おまけ」扱い                                                           |
...
| `KITTAN_MAX_*`      |      | 500字 / 20件 / 640tok / 32tok | 各種上限の上書き                                                  |
...
入力(システムプロンプト+履歴)≒ 数千トークン、出力 640 + 分類 32 トークン。
