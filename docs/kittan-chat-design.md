...
一方で、③の呼び出し自体が失敗した場合(SDKが例外を投げた・APIキーが無い・上流が過負荷)は
安全ブロックではなく**再試行可能なエラー**として扱い、`status: 'error'`(HTTP 500 / 503)を返します。
安全上の停止と生成の失敗は、利用者から見ても(お断り文か再試行可能なエラーか)区別できる別物です。
...
モデル既定のフィルタも「おまけ」と位置づけ、自前の多層防御で「絶対に」を担保します。
想定する脅威は次のとおりです。
