...
差し替えの元データには X の公式アーカイブを使い、取り込みは**候補生成 → 人間のレビュー**の2段階に
分けています。`scripts/import-x-corpus.ts`(判定ロジックは `lib/kittan/xArchive.ts`)は
`corpus.candidates.json` を書き出すだけで、**`corpus.json` を直接書き換えません**。上の運用ルールは
機械的なフィルターでは担保できず、最後は人間が読んで選ぶ必要があるからです。
そのフィルターの一段として、コーパス由来のテキストにも**入力と同じ `BLOCKLIST` を適用**しています
(コーパスはシステムプロンプトに入る = 出力されうるため、入力と同じ基準で足切りします)。
なお、アーカイブのZIPにはDMなど公開してはいけないデータも含まれるため、
展開先(`data/kittan/x-archive/`)と候補ファイルはどちらも `.gitignore` 済みです。
...
| `KITTAN_MODERATION` |      | 有効                             | `0`/`false`/`off`/`no` で層5を無効化(開発用。本番では常に有効)    |
